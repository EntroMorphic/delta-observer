...
Deep neural networks have achieved remarkable success across diverse domains, yet their internal representations remain largely opaque. Feature visualization \citep{olah2017feature} has illuminated what individual neurons detect, while mathematical frameworks for circuit analysis \citep{elhage2021mathematical} have enabled systematic study of computational mechanisms. Recent work has made significant progress in tracking specific phenomena like grokking \citep{nanda2023progress} and scaling interpretability to frontier models \citep{templeton2024scaling, bills2023language}. However, these approaches share a critical limitation: they analyze \textit{final} trained models. What happens \textit{during} training is invisible.

A fundamental question persists: when different architectures solve the same task, do they learn similar or distinct internal representations? Representation similarity methods like SVCCA \citep{raghu2017svcca} and CKA \citep{kornblith2019similarity, morcos2018insights} compare learned representations across models, while linear probing \citep{alain2016understanding, belinkov2017neural, hewitt2019structural, tenney2019bert} tests what information is linearly accessible. Yet these methods operate on static snapshots of trained networks.
...
The mechanistic interpretability program seeks to reverse-engineer neural networks into human-understandable components \citep{olah2017feature, elhage2021mathematical}. This includes studying circuits that implement specific computations \citep{nanda2023progress}, extracting interpretable features from large models \citep{templeton2024scaling}, and using language models to explain neuron behavior \citep{bills2023language}. While powerful, these approaches analyze trained models---our work reveals that important structure exists only \textit{during} training.

\subsection{Linear Probing and Representation Analysis}

Linear probes \citep{alain2016understanding} test whether information is linearly accessible in neural representations. This technique has been applied extensively to language models \citep{belinkov2017neural, hewitt2019structural, tenney2019bert}. Our observation that $R^2=0.9879$ for semantic prediction aligns with findings that semantic information is often linearly accessible, but our discovery that geometric clustering is transient complicates the interpretation of such results.

\subsection{Representation Similarity}

Methods for comparing representations across networks include Singular Vector Canonical Correlation Analysis (SVCCA) \citep{raghu2017svcca}, Centered Kernel Alignment (CKA) \citep{kornblith2019similarity}, and related approaches \citep{morcos2018insights}. These methods typically compare final representations; our online observation approach extends this to the full training trajectory.

\subsection{Manifold Hypothesis and Geometric Structure}

The manifold hypothesis \citep{bengio2013representation, fefferman2016testing} suggests neural networks learn low-dimensional manifolds in high-dimensional space. Geometric deep learning \citep{bronstein2021geometric} exploits this structure. Our finding that geometric clustering is transient suggests the manifold structure itself may be dynamic during training.

\subsection{Learning Dynamics}

Work on loss landscape geometry \citep{frankle2020linear, fort2020deep} studies how networks traverse parameter space during training. Our contribution is analogous but focuses on \textit{representation} space: how the internal organization of information evolves, not just how weights change.

\subsection{Compositional Architectures}

Building networks with compositional inductive biases \citep{lake2017building, battaglia2018relational} is an active research direction. Neural module networks \citep{andreas2016neural} and recurrent independent mechanisms \citep{goyal2019recurrent} explicitly encourage modular computation. We study how compositional vs. monolithic architectures represent the same task differently.

\subsection{Disentangled Representations}

Research on disentanglement \citep{higgins2017beta, chen2018isolating} aims to learn representations where factors of variation are separated. \citet{locatello2019challenging} showed unsupervised disentanglement is fundamentally difficult without inductive biases. Our semantic bottleneck provides such a bias through the prediction objective.

\subsection{Contrastive and Self-Supervised Learning}

Contrastive methods \citep{oord2018representation, chen2020simple, he2020momentum, grill2020bootstrap} learn representations by comparing similar and dissimilar examples. Our cross-network approach shares the spirit of learning shared structure across different ``views''---but our views are different architectures rather than data augmentations.

% ============================================================================
% SECTION 3: THE PROBLEM (ACCESSIBLE VERSION)
% ============================================================================
...
We study a simple but revealing task: \textbf{4-bit binary addition}. Given two 4-bit numbers (0-15 each), predict their 5-bit sum (0-30). This task has been studied in the context of neural arithmetic \citep{trask2018neural, madsen2020neural}, mathematical reasoning \citep{saxton2019analysing}, and learning to execute \citep{zaremba2014learning}.
...
\item \textbf{Compositional Network:} Processes each bit position with a separate module, passing carry information between them---mimicking how humans (and hardware) actually do addition. This reflects compositional generalization principles \citep{lake2018generalization, kim2020cogs}.
\end{itemize}
...
The Delta Observer architecture draws inspiration from variational autoencoders \citep{kingma2013auto, rezende2014stochastic} but with a crucial difference: instead of encoding data, we encode \textit{activations from other networks}. The architecture has three jobs:
...
\subsection{Measuring Semantic Structure}

We use two complementary metrics:

\textbf{Linear Accessibility ($R^2$):} We fit a linear regression from the latent space to carry count. High $R^2$ means the semantic variable is linearly decodable---a standard test in probing literature \citep{alain2016understanding}.

\textbf{Geometric Clustering (Silhouette):} The Silhouette score \citep{rousseeuw1987silhouettes} measures whether points cluster by their semantic label. Positive values indicate clustering; near-zero indicates no geometric organization.
...
Current interpretability methods---probing classifiers \citep{alain2016understanding, belinkov2017neural}, activation visualization \citep{olah2017feature}, circuit analysis \citep{elhage2021mathematical, nanda2023progress}---all operate on final trained models. Our results suggest this misses crucial dynamics. The most informative structure may be \textbf{transient}.

This connects to broader concerns about interpretability methods. Just as \citet{locatello2019challenging} showed that disentanglement claims require careful validation, our work suggests that claims about geometric structure in representations require attention to \textit{when} during training the analysis is performed.
...
This perspective connects to work on loss landscape geometry \citep{frankle2020linear, fort2020deep}, but shifts focus from parameter space to representation space. The relevant question becomes: what information is encoded in the \textit{path} through representation space, not just the endpoint?
...
\item \textbf{Architecture Dependence:} We studied MLP and compositional architectures; whether transformers \citep{elhage2021mathematical} or other architectures exhibit similar dynamics is unknown.
\end{itemize}
...
\item \textbf{Preserve Scaffolding:} Can we modify training to \textit{keep} the geometric clusters? Would this improve interpretability? Regularization approaches from disentanglement research \citep{higgins2017beta, chen2018isolating} may be relevant.
...
\item \textbf{Connection to Grokking:} The phenomenon of grokking \citep{nanda2023progress}---sudden generalization after apparent convergence---may be related to transient geometric structure. Do grokking networks show similar clustering dynamics?
\end{enumerate}
